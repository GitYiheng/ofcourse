...
class Order:
    ...
    def select_service_level(self, service_level):
        ...
        if service_level == ServiceLevel.premium:
            self.max_price = 50
            self.max_time = 10
        elif service_level == ServiceLevel.standard:
            self.max_price = 40
            self.max_time = 20
        elif service_level == ServiceLevel.economy:
            self.max_price = 30
            self.max_time = 30
        else:
            raise ValueError(f"Unsupported service level: {service_level!r}")
    ...
    def update(self, specified_price=None, specified_time=None):
        _step_price = specified_price if specified_price is not None else self.recurring_price
        self.cumulative_price += _step_price
        self.cumulative_time += specified_time if specified_time is not None else self.recurring_time
        return _step_price
    ...
    def _assign_reference_number(self, reference_number):
        self.reference_number = reference_number if reference_number is not None else id(self)
