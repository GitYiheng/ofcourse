...
class PPOAgents(BaseAgents):
    ...
    def generate_rollouts(self, explore=True, logger=None, summary=None):
        ...
        if logger:
            logger.info("\n")
        batch_size = min(self.args.batch_size, len(rollout_observations_th))
        batch_indices = random.sample([*range(len(rollout_observations_th))], batch_size)
    ...
    def train(self, batch_log_probs_th, batch_advantages_th, batch_values_th, batch_returns_th):
        batch_size = len(batch_log_probs_th)
        batch_actor_loss_th = [torch.zeros((), dtype=torch.float32) for step in range(batch_size)]

        for t in reversed(range(batch_size)):
            batch_actor_loss_th[t] = -(batch_log_probs_th[t] * batch_advantages_th[t])

        actor_loss_th = torch.stack([batch_actor_loss_th[t] for t in range(batch_size)]).mean()
    ...
    def learn(self, logger=None, summary=None):
        ...
        progress = tqdm(range(self.args.n_epoch), desc="Training PPO", unit="epoch")
        for t_epoch in progress:
            ...
            if logger and summary:
                ...
                progress.set_postfix(
                    ...
                    actor_loss=self.format_progress_value(actor_loss_th.item()),
    ...
    def evaluate(self, logger=None, summary=None):
        progress = tqdm(range(self.args.n_epoch), desc="Evaluating PPO", unit="epoch")
        for t_epoch in progress:
            batch_observations_th, batch_actions_th, batch_rewards_th, batch_dones_th, batch_values_th, batch_log_probs_th, batch_returns_th, batch_advantages_th = self.generate_rollouts(
